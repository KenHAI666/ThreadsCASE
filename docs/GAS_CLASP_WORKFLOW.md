...
新版契約名稱為 `scan_unique_v3`，舊版待同步事件仍以 `scan_unique_v2` 相容處理。V1.1.5 不更動既有 Threads DOM 抓取、IndexedDB 保存、匯出與冒險者核心功能；只修正登入狀態與商店版橋接優先順序，並沿用後台批次用量驗證與確認結果同步。
